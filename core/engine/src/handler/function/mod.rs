...
impl FunctionHandler {
    ...
    pub async fn handle(&self, request: NodeRequest) -> NodeResult {
        ...
        let nodes_data = request.input.dot("$nodes").unwrap_or_default();
        ...
        self.attach_globals(nodes_data.clone())
            .await
        ...
        let response_result = self
            ...
            .call_handler(
                &module_name,
                JsValueWithNodes(JsValue(request.input.clone())),
            )
            .await;
    ...
    async fn attach_globals(&self, nodes_data: Variable) -> FunctionResult {
