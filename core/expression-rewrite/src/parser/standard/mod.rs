...
impl<'arena, 'token_ref> StandardParser<'arena, 'token_ref> {
    ...
    fn expression(&self, precedence: u8) -> ParserResult<&'arena Node<'arena>> {
    ...
    fn primary_expression(&self) -> ParserResult<&'arena Node<'arena>> {
        let token = self.iterator.current();
        if token.kind == TokenKind::Operator {
    ...
    fn conditional(&self, node: &'arena Node<'arena>) -> ParserResult<&'arena Node<'arena>> {
        let mut nd = self.iterator.node(node.clone())?;
        let mut expr1: &'arena Node;
        let mut expr2: &'arena Node;
    ...
    fn interval(&self) -> ParserResult<Option<&'arena Node<'arena>>> {
    ...
    fn identifier_expression(&self, token: &Token<'arena>) -> ParserResult<&'arena Node<'arena>> {
        if self.iterator.current().kind != TokenKind::Bracket
            || self.iterator.current().value != "("
        {
            return self.iterator.node(Node::Identifier(token.value));
        }
    ...
    fn array(&self, _token: &Token) -> ParserResult<&'arena Node<'arena>> {
    ...
    fn closure(&self) -> ParserResult<&'arena Node<'arena>> {
    ...
    fn postfix(&self, node: &'arena Node<'arena>) -> ParserResult<&'arena Node<'arena>> {
        ...
        while postfix_token.kind == TokenKind::Bracket || postfix_token.kind == TokenKind::Operator
        {
            if postfix_token.value == "." {
                ...
                let property = self.iterator.node(Node::String(property_token.value))?;
