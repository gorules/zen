...
pub struct UnaryParser<'arena, 'token_ref> {
...
impl<'arena, 'token_ref> UnaryParser<'arena, 'token_ref> {
    ...
    fn expression(&self, precedence: u8, root: bool) -> ParserResult<&'arena Node<'arena>> {
        let mut node_left: &'arena Node<'arena> = self.iterator.node(MAIN_NODE)?;
        ...
        while !self.iterator.is_done() {
            match token.kind {
                TokenKind::Operator => {
                    if token.value == "," {
                        self.iterator.next()?;
                        let node_right = self.expression(0, true)?;
    ...
    fn interval(&self, node: &'arena Node<'arena>) -> ParserResult<Option<&'arena Node<'arena>>> {
    ...
    fn array(&self, _token: &Token<'arena>) -> ParserResult<&'arena Node<'arena>> {
