    ...
    pub fn run(&mut self, root_env: Variable) -> VMResult<Variable> {
        ...
        while self.ip < self.bytecode.len() as u32 {
            ...
            match op {
                ...
                Opcode::AssignedObjectEnd => {
                    let Some(mut assigned_object) = assigned_object.take() else {
                        return Err(OpcodeErr {
                    ...
                    if let Some(root_object) = assigned_object.dot("$root") {
                        assigned_object = root_object;
                    }
